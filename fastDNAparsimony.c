...
static parsimonyNumber *compressDNA(tree *tr, int *informative)
{
  ...
  for(i = 0; i < (size_t)tr->rdta->sites; i++)    
    if(informative[i])
      entries += 1;     
  ...
  for(i = 0; i < (size_t)tr->mxtips; i++)
	...
	*compressedTip_T = &compressedScratch[(compressedEntriesPadded * 4 * (i + 1)) + compressedEntriesPadded * 3],
	...
	index = 0;
      ...
      for(index = 0; index < (size_t)tr->rdta->sites; index++)
	{
...
int makeParsimonyTreeFastDNA(tree *tr, analdef *adef, int startSite, int endSite)
{   
  nodeptr  
    p, 
    f;    
  ...
  int 
    ...
    *informative = (int *)malloc(sizeof(int) * (size_t)tr->rdta->sites);  
  ...
  int  minParsimonyScoreOverRuns = INT_MAX;
...
/*	char 
  ...
  return  minParsimonyScoreOverRuns ;//tr->bestParsimony; 
